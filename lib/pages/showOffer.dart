...
class ShowOffer extends StatefulWidget {
  final String id;
  ShowOffer({super.key, required this.id});
  @override
...
class _ShowOfferState extends State<ShowOffer> {
  ...
  void initState() {
      ...
      readDB(Entity.shop, widget.id).then((value){
      int length = value['offers'].length;
      List<MapEntry<String, dynamic>> offersList = value['offers'].entries.toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar:
      PreferredSize(
          preferredSize: Size.fromHeight(50.0), // Decrease the overall height here
          child: AppBar(
            ...
            title: Container(
              padding: EdgeInsets.only(top: 5.0), // Adjust padding as needed
              child: Text('Offers', style: TextStyle(fontSize: 18.0)), // Maintain text size
            ),
            leading: Container(
              padding: EdgeInsets.only(top: 5.0), // Adjust padding as needed
              child: IconButton(
